...
From the scatter plot of BMI vs FEV, there does appear to be a positive correlation where as BMI increases, so does forced expiratory volume.
...
The map shows greater PM 2.5 concentrations in the Mira Loma area with a concentration of 29.97 µg/m3. Lompoc seems to have the lowest PM 2.5 concentration with 5.96 µg/m3. 
```{r}
library(leaflet)
leaflet(merged_data) |>
addTiles() |>
...
The scatterplot shows that there seems to be a slightly negative association between pm2.5 mass and FEV. We can see that as mass increases, FEV slightly decreases. 
